...
class SeleniumServer(GenericTool.Tool):
    ...
    def __startProcess(self, timeout=20):
        ...
        try:
            if sys.platform == "win32" :
                __cmd__ = BIN_WIN
                __cmd__ += r' -Dwebdriver.ie.driver="%s\Plugins\selenium3server\bin\IEDriverServer.exe" ' % (
                                                                                    Settings.getDirExec()
                                                                                )
                __cmd__ += r' -Dwebdriver.chrome.driver="%s\Plugins\selenium3server\bin\chromedriver.exe" ' % (
                                                                                Settings.getDirExec()
                                                                                )
                __cmd__ += r' -Dwebdriver.opera.driver="%s\Plugins\selenium3server\bin\operadriver.exe" ' % (
                                                                                Settings.getDirExec()
                                                                                )
                __cmd__ += r' -Dwebdriver.gecko.driver="%s\Plugins\selenium3server\bin\geckodriver.exe" ' % (
                                                                                Settings.getDirExec()
                                                                                )
                __cmd__ += r' -Dwebdriver.edge.driver="%s\Plugins\selenium3server\bin\MicrosoftWebDriver.exe" ' % (
                                                                                Settings.getDirExec()
                                                                                )
